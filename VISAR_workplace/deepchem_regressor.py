...
class deepchem_robust_regressor(visar_model):
    ...
    def fit(self, dataset):
        train_evaluation = [dataset.get_task_names()]
        test_evaluation = [dataset.get_task_names()]
        for iteration in range(self.epoch_num):
            model.fit(train_dataset, n_epoch = self.epoch, max_checkpoints_to_keep = 1, checkpoint_interval=20)
            print('======== Iteration %d ======' % iteration)
            print("Evaluating model")
            train_scores = self.evaluate(train_dataset)
