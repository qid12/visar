...
class visar_model:
    ...
    def generate_compound_df(self, data_loader, df, coord_values, id_field, valid_mask = None):
        pred_mat = self.predict(data_loader)
        if not valid_mask is None:
            pred_mat = pred_mat[:,valid_mask]
        pred_df = pd.DataFrame(pred_mat)
        pred_df.columns = ['pred_' + self.para_dict['task_list'][0]]
    ...
    def generate_viz_results(self, train_loader, train_df, output_prefix,
        ...
        compound_df.to_csv(self.model_path + output_prefix + 'compound_df.csv', index = False)
        batch_df.to_csv(self.model_path + output_prefix + 'batch_df.csv', index = False)
        task_df.to_csv(self.model_path + output_prefix + 'task_df.csv', index = False)
    ...
    def generate_instance_analysis(self, smiles_string, prev_model, valid_mask,
